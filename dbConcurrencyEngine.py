...
from GlobalLockIndex import GlobalLockIndex
...
class dbConcurrencyEngine:
    ...
    def __init__(self, incoming_query_queues, used_a_query_cv, run_concurrency_check, query_completed_condition):
        ...
        self.sidetrack_index=SidetrackIndex()
      ...
      for new_query in new_queries:
          ...
          if self.run_concurrency_check and not readonly and self.lock_index.does_conflict(new_query):
              ...
              if remove_from_sidetrack:
                  self.remove_from_sidetrack(new_query)
...
# Try sidetracked queries again
    ...
    def remove_from_sidetrack(self, query):
...
# Wait for all admitted (non-conflicting) queries to finish and process them
    def wind_down(self):
      print("  Winding down. {}  Total:{}, Completed:{}".format(time.time(),self.query_count, self._total_completed_queries))
      ...
      self.proccess_completed_queries()
      print("  Wound down. {}  Total:{}, Completed:{}".format(time.time(),self.query_count, self._total_completed_queries))
    ...
    def consider_changing_lock_mode(self, max_sidetracked_queries, minimum_queue_size, target_queue_size):
      ...
      if self.run_concurrency_check == True:
        if len(self.sidetrack_index) > max_sidetracked_queries:
          ...
          admitted_queries = self.append(queries, place_on_sidetrack=True, remove_from_sidetrack=False, readonly=True)
          ...
          for combination in lock_combinations:
              ...
              if len(value) > minimum_queue_size:
                ...
                while len(queries)>target_queue_size:
                  self.proccess_completed_queries()
                  admit_loops+=1
                  queries_admitted = self.append(queries, place_on_sidetrack=False, remove_from_sidetrack=True)
                  admitted += len(queries_admitted)
                  for q in queries_admitted:
                    queries.remove(q)
                ...
                end_depth=len(value)
                print("Admitted {} of {} queries isolated by columns {}.".format(admitted, query_count, ",".join(combination)))
          
          # After looping over these return to standard lock mode by default
      self.lock_index.set_scheduled_columns({})

class SidetrackIndex:
  def __init__(self):
    ...
    if query.readonly:
      ...
      for index_name, value in query.lock_indexes.items(): #Loop over indexes in query
        for column in query.lock_indexes[index_name]: # Loop over columns in query
          if column in self.sidetrack_indexes[index_name] and query in self.sidetrack_indexes[index_name][column]:
