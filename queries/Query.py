    ...
    def add_comparison(self, comparison, mode):
        ...
        if '.' in comparison_column:
            ...
            self.predicatelock.add_value(main_column,
                                         Parser.comparison_types(self, str(comparison[2])),
                                         comparison_column,
                                         mode)
...
class Parser:
    @staticmethod
    def generate_locks(query, skip_read_only):
        if query.sql_obj.get_type() == 'SELECT':
            query.readonly = True
            if skip_read_only:
                return
            for token in query.sql_obj.tokens:
                if type(token) is sqlparse.sql.Where:
                    for predicate in token:
                        if type(predicate) is sqlparse.sql.Comparison:
                            query.add_comparison(predicate, PredicateLock.READ)
                        if type(predicate) is sqlparse.sql.Parenthesis:
                            for n_predicate in predicate.tokens:
                                if type(n_predicate) is sqlparse.sql.Comparison:
                                    query.add_comparison(n_predicate, PredicateLock.READ)
                ...
                if in_identifier_list and type(token) is sqlparse.sql.IdentifierList:
                    for identifier in token:
                        if type(identifier) is sqlparse.sql.Identifier:
                            if not query.predicatelock.value_exists(PredicateLock.READ, str(identifier)):
                                query.add_identifier(identifier, PredicateLock.READ)
                if in_identifier_list and type(token) is sqlparse.sql.Identifier:
                    if not query.predicatelock.value_exists(PredicateLock.READ, str(token)):
                        query.add_identifier(token, PredicateLock.READ)

        if query.sql_obj.get_type() == 'UPDATE':
            for token in query.sql_obj.tokens:
                if type(token) is sqlparse.sql.Where:
                    for predicate in token:
                        if type(predicate) is sqlparse.sql.Comparison:
                            query.add_comparison(predicate, PredicateLock.WRITE)
                        if type(predicate) is sqlparse.sql.Parenthesis:
                            for n_predicate in predicate.tokens:
                                if type(n_predicate) is sqlparse.sql.Comparison:
                                    query.add_comparison(n_predicate, PredicateLock.WRITE)
                ...
                if in_set_list and type(token) is sqlparse.sql.IdentifierList:
                    for comparison in token:
                        if type(comparison) is sqlparse.sql.Comparison:
                            if not query.predicatelock.value_exists(PredicateLock.WRITE, str(comparison[0])):
                                query.add_identifier(comparison[0], PredicateLock.WRITE)
                            query.add_comparison(comparison, PredicateLock.WRITE)

                if in_set_list and type(token) is sqlparse.sql.Comparison:
                    if not query.predicatelock.value_exists(PredicateLock.WRITE, str(token[0])):
                        query.add_identifier(token[0], PredicateLock.WRITE)
                    query.add_comparison(token, PredicateLock.WRITE)

        if query.sql_obj.get_type() == "INSERT":
            for token in query.sql_obj.tokens:
                                ...
                                if type(list) is sqlparse.sql.IdentifierList:
                                    for id in list.tokens:
                                        if type(id) is sqlparse.sql.Identifier:
                                            query.add_identifier(id, PredicateLock.WRITE)
            for token in query.sql_obj.tokens:
                if type(token) is sqlparse.sql.Where:
                    for predicate in token:
                        if type(predicate) is sqlparse.sql.Comparison:
        ...
        if query.sql_obj.get_type() == "DELETE":
            for token in query.sql_obj.tokens:
    ...
    @staticmethod
    def comparison_types(query, comparison_string):
